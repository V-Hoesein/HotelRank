...
async def main():
    ...
    with open("agoda_search_response.json", "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)
    print("💾 Response disimpan ke agoda_search_response.json")

    # Simpan juga payload asli ke file untuk referensi pembuatan CUSTOM_PAYLOAD
    if result.get("original_payload"):
        with open("agoda_original_payload.json", "w", encoding="utf-8") as f:
            try:
                f.write(json.dumps(json.loads(result["original_payload"]), indent=2, ensure_ascii=False))
            except Exception:
                f.write(result["original_payload"])
        print("💾 Payload asli disimpan ke agoda_original_payload.json")
    ...
    if isinstance(data, dict):
        ...
        if not properties:
            print("\n⚠️  Properties kosong — lihat agoda_search_response.json untuk memeriksa struktur response.")
            return
