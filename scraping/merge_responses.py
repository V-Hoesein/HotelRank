...
merge_responses.py
──────────────────
Membaca semua file response_*.json di folder results/raw_graphql/,
mengekstrak array dari data.citySearch.properties,
menggabungkan semuanya menjadi 1 array unik berdasarkan propertyId,
lalu menyimpan ke results/search_results/result_<keyword>.json
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "data", "raw")
...
def main(keyword: str = KEYWORD):
    ...
    print(f"{'='*55}")
    print(f"[MERGE] Response -> {OUTPUT_DIR}/result_{keyword}.json")
    print(f"{'='*55}")
